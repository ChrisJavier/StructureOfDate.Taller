public class SLinkedDouble {
	...
	{
		size=0;
		header= new Node(null, null,0,null);
		trailer= new Node(null, null, 0,null);
		header.setNext(trailer);
	}

	public void addNode(String Name){
		
		Node New =  new Node(null,null,0,null);
		Node Last = new Node(null,null,0,null);
		if(IsEmpty()){
			New.setName(Name);
			header=New;
		}else{
...
public void AddLast(int dato){
	...
	public Node Search(int Ci){
		Node aux= new Node(null,null,0,null);
		Node Find=new Node(null,null,0,null);
	...
	public void Listar(){
		if(!IsEmpty()){
			...
			System.out.println("#Producto" +"  "+ "[      Cantidad    ]"+"[   Descripcion   ]");
			while(i!=size){
				if(Aux.getCantidad()!=0){
					System.out.println(i +"-> "+ "                  [ "+Aux.getCantidad()+" ]"+"        [ "+Aux.getName()+" ]"+"     Listo!!!");	
				}			
			Aux= Aux.getNext();
			i++;
			}
			...
			while(i!=size){
				if(Aux.getCantidad()==0){
					System.out.println(i +"-> "+ "                  [ "+Aux.getCantidad()+" ]"+"        [ "+Aux.getName()+" ]");	
				}			
			Aux= Aux.getNext();
			i++;
			}
	...
	public void InsertNewElementPerRefBefore(int dato,int ref){
		...
		Node New= new Node(null,null,0,null);
		Node Aux=header;
		Node Aux1=new Node(null,null,0,null);
	...
	public void Remove(int dato){
		...
		if(aux.getCantidad()==header.getCantidad()){
			...
			Node Aux1= new Node(null,null,0,null);
			Node Aux2= new Node(null,null,0,null);
